        ...
        <section ref="detailSectionRef" class="card">
          ...
          <Transition name="report-detail-swap" mode="out-in">
            ...
            <div v-else-if="reportDetail" :key="`detail-${reportDetail.report_id}`" class="stack">
              ...
              <div v-if="reportHourlyData.length" class="box"><div class="title"><Icon icon="lucide:clock" />小时数据</div><table class="table"><thead><tr><th>时间</th><th>总能耗</th><th>峰值</th><th>平均</th></tr></thead><TransitionGroup name="table-rise" tag="tbody" class="table-rise-body"><tr v-for="(item, idx) in reportHourlyData" :key="item.hour || idx" :style="{ transitionDelay: `${Math.min(idx, 8) * 28}ms` }"><td>{{ item.hour }}</td><td>{{ formatNumber(item.total) }}</td><td>{{ formatNumber(item.peak) }}</td><td>{{ formatNumber(item.average) }}</td></tr></TransitionGroup></table></div>
