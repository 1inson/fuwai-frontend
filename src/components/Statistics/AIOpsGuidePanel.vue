          ...
          <div class="body">
            <section class="card">
              <div class="section-top">
                <div>
                  <h3>报表导出</h3>
                  <p>创建时仅通过 `include_ai_summary` 标记是否需要 AI 建议，已有报表分析走 `/ai/report-summary`。</p>
                </div>
              ...
              <div v-if="reportOpen" class="stack">
                <div class="grid">
                  <label><span>报表类型</span><select v-model="form.report_type"><option v-for="item in reportTypeOptions" :key="item.value" :value="item.value">{{ item.label }}</option></select></label>
                  <label><span>建筑 ID</span><input :value="buildingId" disabled /></label>
                  <label><span>开始时间</span><input v-model="form.start" type="datetime-local" /></label>
                  <label><span>结束时间</span><input v-model="form.end" type="datetime-local" /></label>
                </div>

                <label class="toggle-row">
                  <div><strong>包含 AI 建议</strong><small>传参字段：`include_ai_summary`</small></div>
                ...
                <div v-if="reportDetail" class="result stack-sm">
                  <div class="kv"><span>任务状态</span><strong>{{ reportDetail.status }}</strong></div>
                  <div class="kv"><span>报表 ID</span><strong>{{ reportDetail.report_id }}</strong></div>
                  <div class="actions">
                    <button class="primary" :disabled="reportDetail.status !== 'ready'" @click="downloadReportFile"><Icon icon="lucide:download" />下载 Markdown</button>
                    <button class="secondary" :disabled="summarizing || !reportDetail.report_id" @click="analyzeReport"><Icon :icon="summarizing ? 'lucide:loader-2' : 'lucide:sparkles'" :class="{ spin: summarizing }" />{{ summarizing ? '分析中...' : 'AI 分析已有报表' }}</button>
                  </div>
                </div>

                <div v-if="reportSummary" class="summary-box">
                  <div class="title"><Icon icon="lucide:sparkles" />AI 报表建议</div>
                  <p>{{ reportSummary }}</p>
                </div>

                <div class="actions">
                  <button class="secondary" :disabled="generating" @click="resetReportForm">重置参数</button>
                  <button class="primary" :disabled="generating" @click="generate"><Icon :icon="generating ? 'lucide:loader-2' : 'lucide:file-output'" :class="{ spin: generating }" />{{ generating ? '生成中...' : '生成并导出报表' }}</button>
                </div>
              </div>
            </section>
            ...
            <div v-else-if="opsResult" class="stack">
              ...
              <section v-if="opsResult.preconditions?.length" class="card"><h4>前置条件</h4><ul><li v-for="(item, index) in opsResult.preconditions" :key="index">{{ item }}</li></ul></section>
              <section v-if="opsResult.steps?.length" class="card"><h4>运维步骤</h4><div class="stack-sm"><div v-for="(step, index) in opsResult.steps" :key="step.step_id" class="step"><div class="step-head"><span class="badge">{{ index + 1 }}</span><strong>{{ step.title }}</strong><em>{{ step.priority }}</em></div><p>{{ step.instruction }}</p><small>预期结果：{{ step.expected_result }}</small><small>未达标处理：{{ step.if_not_met }}</small></div></div></section>
              <section v-if="opsResult.risk_notice?.length" class="card"><h4>风险提示</h4><ul class="risk"><li v-for="(item, index) in opsResult.risk_notice" :key="index">{{ item }}</li></ul></section>
              <section v-if="opsResult.actions?.length" class="card"><h4>建议操作</h4><div class="actions"><button v-for="(action, index) in opsResult.actions" :key="index" class="primary small"><Icon icon="lucide:arrow-right" />{{ action.label }}</button></div></section>
              <section v-if="opsResult.evidence?.length" class="card"><h4>相关依据</h4><div class="stack-sm"><div v-for="(item, index) in opsResult.evidence" :key="index" class="evidence"><div><strong>{{ item.source }}</strong><em>{{ item.source_type }}</em></div><p>{{ item.snippet }}</p></div></div></section>
              <section v-if="opsResult.applicability" class="card"><h4>适用范围</h4><div class="stack-sm"><div v-if="opsResult.applicability.applies_to?.length" class="tags"><strong>适用</strong><span v-for="(item, index) in opsResult.applicability.applies_to" :key="index">{{ item }}</span></div><div v-if="opsResult.applicability.not_applies_to?.length" class="tags"><strong>不适用</strong><span class="bad" v-for="(item, index) in opsResult.applicability.not_applies_to" :key="index">{{ item }}</span></div></div></section>
              <section class="meta"><span><Icon icon="lucide:cpu" />{{ opsResult.meta?.model || '-' }}</span><span><Icon icon="lucide:clock" />{{ formatGeneratedAt(opsResult.meta?.generated_at) }}</span><span v-if="opsResult.meta?.knowledge_hits != null"><Icon icon="lucide:book-open" />知识命中 {{ opsResult.meta.knowledge_hits }}</span><span v-if="opsResult.meta?.history_feedback_hits != null"><Icon icon="lucide:history" />历史反馈 {{ opsResult.meta.history_feedback_hits }}</span></section>
            </div>
            ...
            <div v-else class="card center">
              ...
              <button class="primary" @click="startAnalysis"><Icon icon="lucide:sparkles" />开始分析</button>
            </div>
          </div>
        </aside>
      </div>
...
const handleOpsSSEEvent = (event: OpsGuideSSEEvent) => {
  ...
  switch (type) {
    ...
    case 'progress':
      ...
      break
    case 'summary':
      if (!opsResult.value) opsResult.value = { incident_id: '', status: 'streaming', summary: data?.summary || data || '', steps: [], meta: { generated_at: new Date().toISOString(), model: '' } }
      else opsResult.value.summary = data?.summary || data || ''
      break
    case 'preconditions':
...
const cancelOpsGuide = () => {
  if (opsAbortController.value) { opsAbortController.value.abort(); opsAbortController.value = null }
  opsLoading.value = false
  opsProgressMsg.value = ''
}
